...
- Session 3: Branching guide (main/dev workflow), doc sync, branching setup verification
...
### Branching guide and workflow — Session 3
- **What:** Created `docs/BRANCHING-GUIDE.md` — reference for main/dev branching, PR-based merges, release tagging, optional staging app for Streamlit Cloud.
- **Why:** Adopt main = production (Streamlit deploys), dev = working branch. Merge via PR for documented history. Tag releases on main.
- **Doc sync:** Updated DOCUMENTATION-GUIDE (project structure, commit workflow), README (Known Issues, Development section, Roadmap), iteration Version Log. Added issue #28 (no structured error logging) to Remaining Issues.
- **Setup verified:** Tag v0.4 on main, dev branch created and pushed. Both branches in sync at commit `78c8535`.
...
| 28 | No structured error logging — errors surface only in Streamlit UI; no centralized log for debugging | Session 2 (module cache incident) |
...
- **2026-02-28** — Created `docs/BRANCHING-GUIDE.md` (main/dev workflow, PRs, tags, optional staging app)
- **2026-02-28** — main/dev branching setup verified (tag v0.4, dev branch from main)
