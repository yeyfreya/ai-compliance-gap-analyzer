...
- **2026-03-01** — Repo restructure (Session 3, v0.5.1): README rewritten product-first (value prop → demo → screenshot → report description). Created `docs/ARCHITECTURE.md` (technical decisions doc). Removed unused test files (`test_all.py`, `test_api.py`, `test_tavily.py`) and test artifact reports from git tracking. Renamed showcase reports to clean filenames (`report_healthcare.md`, `report_fintech.md`, `report_regtech.md`). Gitignored `docs/dev-logs/` (iteration docs remain public). Added `assets/screenshot.jpg`. Updated DOCUMENTATION-GUIDE Section 8 for README structure and voice guidelines.
