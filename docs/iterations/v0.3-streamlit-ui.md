...
- Session 2: Streamlit Cloud deployment
...
## Changes Made During This Version (Session 2 — Streamlit Cloud Deployment)

### Streamlit Cloud secrets support (`streamlit_app.py`)
- **What:** Added secrets injection block before agent module imports. Reads `ANTHROPIC_API_KEY` and `TAVILY_API_KEY` from `st.secrets` and sets them as environment variables so `agent.py` and `tools.py` pick them up via their existing `os.getenv()` calls. Locally, the block is a no-op (`.env` handles it via `load_dotenv()`).
- **Error handling:** If secrets are missing, the app shows a clear error message with the required TOML format instead of crashing with a raw traceback.
- **Why:** Streamlit Cloud doesn't use `.env` files. Secrets are configured via the dashboard and accessed through `st.secrets`, not environment variables. This bridge keeps `agent.py` and `tools.py` unchanged.

### Pinned requirements.txt versions
- **What:** Changed from unpinned (`anthropic`) to minimum-version pins (`anthropic>=0.79.0`). Based on installed versions at time of deployment.
- **Why:** Unpinned deps could break on Streamlit Cloud if a package releases a breaking update. Floor pins (`>=`) ensure compatibility while still allowing newer versions. Resolves issue #11.

### Deployed to Streamlit Cloud
- **What:** App deployed via share.streamlit.io, connected to GitHub repo `yeyfreya/ai-compliance-gap-analyzer`, main file `streamlit_app.py`. Secrets configured via dashboard.
- **Initial error:** `MissingAPIKeyError` on first deploy — secrets weren't being picked up. Fixed by adding the explicit error message so the user could verify secrets format.
...
| 11 | `requirements.txt` has no version pins | Resolved — pinned with `>=` floor versions for cloud deployment |
...
| 20 | Cloud reports are ephemeral (lost on app restart) — users must use download button | Streamlit Cloud deployment |
| 21 | No usage tracking — user wants to log inputs, agent behavior, and generated reports | User request |
| 22 | API cost exposure — public demo has no rate limiting; each run costs ~$0.05–0.15 | Cloud deployment |

### Planned for v0.4
| Feature | Description |
|---|---|
| **Supabase integration** | Persistent database for usage tracking: user inputs, search queries, timing, and full report content. Replaces ephemeral filesystem storage on Streamlit Cloud. |
| **Rate limiting** | Protect API costs on the public demo (access code gate or session limit) |
...
- **2026-02-27** — Added Streamlit Cloud secrets support to `streamlit_app.py` — injects `st.secrets` as env vars before agent import
- **2026-02-27** — Added missing-secrets error message with TOML format hint
- **2026-02-27** — Pinned `requirements.txt` versions with `>=` floor pins (resolves issue #11)
- **2026-02-27** — Deployed to Streamlit Cloud — live public demo
- **2026-02-27** — Identified v0.4 priorities: Supabase for usage tracking, rate limiting for API cost protection
