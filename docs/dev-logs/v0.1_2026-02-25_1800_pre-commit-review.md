...
- Local file paths (project paths, `C:\Users\<username>\...`) exposed in transcripts
...
### Task 8: `.gitignore` Wildcard Fix

**Issue:** `.cursor/` in `.gitignore` blocked the entire directory, so `!.cursor/rules/` negation was ignored — `pre-commit-review.mdc` wasn't being staged.

**Fix:** Changed `.cursor/` to `.cursor/*` (wildcard). Git now ignores the *contents* but still looks inside the directory, allowing the `!.cursor/rules/` exception to work.

---

### Task 9: Pre-Commit Hook Test & False Positive Fixes

**What happened:** Ran `git commit` to test the hook. It blocked the commit with two findings:
- "Possible API keys or secrets" — the dev log text mentioned literal API key pattern strings
- "Personal email found" — both the Cursor rule and dev log contained the literal email for documentation purposes

**Fix:** Removed literal patterns from both files — replaced with generic descriptions that convey the same meaning without triggering the hook.

**Outcome:** Second commit attempt passed the hook cleanly. Committed and pushed to GitHub successfully.
...
3. `.cursor/` protected except for `rules/` directory (use `.cursor/*` not `.cursor/`)
...
7. Avoid literal secret patterns and personal info in committed files — even in documentation

## Files Modified
- `.gitignore` — added `.cursor/*`, `!.cursor/rules/`, `docs/dev-logs/transcripts/`
- `agent.py` — fixed bare `except`, fixed indentation
- `docs/DOCUMENTATION-GUIDE.md` — updated transcript section, added `HHMM` to naming conventions, updated project structure tree
- `.cursor/rules/pre-commit-review.mdc` — new Cursor rule for pre-commit checklist (redacted literal email)
- `.git/hooks/pre-commit` — new git hook (local-only, not committed)
- `docs/dev-logs/v0.1_2026-02-25_1800_pre-commit-review.md` — this file (redacted literal patterns)
